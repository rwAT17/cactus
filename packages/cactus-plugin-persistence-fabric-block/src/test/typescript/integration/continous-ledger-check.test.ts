...
describe("Persistence Fabric", () => {
  ...
  beforeAll(async () => {
    ...
    // #useridentity test
    const userIdentity = await enrollUser(
      connectionProfile,
      tmpWalletDir,
      ledgerUserName,
      adminName,
    ...
    const keychainPlugin = new PluginKeychainMemory({
      ...
      keychainId,
      logLevel: sutLogLevel,
      backend: new Map([[keychainEntryKey, JSON.stringify([userIdentity])]]), // [userIdentity] - test value, default is userIdentity
    });
  ...
  test("plugins checks", async () => {
    log.warn("###SigningCredential", signingCredential);
    log.warn("###ApiClient", apiClient);
    log.warn("###Persistence", persistence);
    // log.debug("###gateway", gatewayOptions);
  });
  test("plugins checks", async () => {
    const response = await persistence.getBlockFromLedger("1");
