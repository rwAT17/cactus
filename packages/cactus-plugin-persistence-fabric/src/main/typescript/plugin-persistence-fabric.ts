...
export class PluginPersistenceFabric
  ...
  public async lastBlockInLedger(
    ...
    const lastBlockInChain = await this.apiClient.runTransactionV1({
    ...
    this.log.info(
      "lastblock just FunctionOutput:",
      lastBlockInChain.data.functionOutput,
    );
    this.lastBlock = Number(lastBlockInChain.data.functionOutput);
